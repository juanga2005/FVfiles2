c     this is a version of the calw3ez.f file for solution of Diffusion 
c     equations in 3D using dimensional splitting. 
c
c     This function is not useful for solution of Adv-Diff equation 
c     but it is useful for solution of diffusion equations and testing
c     diffusion solvers.
c
c     
c     This function should be called in a generic driver.f program.
c
c     Bamdad Hosseini, Jul 2012.
c
c     ===============================================================
      subroutine ADIdiff3ez (maxmx, maxmy, maxmz,meqn,
     &                 mbc,maux,mwork,q,work,aux, mwaves,
...
c
c     ###############################################################
c     this block includes parameters that are used by
c     the implicit solver
c     ###############################################################
...
c
c     ################################################################
c      THIS ENTIRE BLOCK IS COPIED FROM CLAW3EZ.F FROM CLAW 4.3 PACKAGE
c     ################################################################
c
...
c     ##############################################################
c     assemble the left hand side matrices for implicit euler solver
c     ##############################################################
c
c     in x direction
c
      ...
      call asmblmat1 (maxmx, mbc, mx, coux, 1, SLHSX, lda) !assemble matrix
      call factormat1(maxmx, mbc, mx, SLHSX, lda, ml, mu, ipvtx, info) !choleski factr
c
c     in y direction
c
      couy = dtv(1)/(dy*dy)
      ...
      call asmblmat1 (maxmy, mbc, mx, couy, 1, SLHSY, lda)
      call factormat1(maxmy, mbc, mx, SLHSY, lda, ml, mu, ipvty, info)
c
c     in z direction
c
      couz = dtv(1)/(dz*dz)
      ...
      call asmblmat1 (maxmz, mbc, mz, couz, 1, SLHSZ, lda)
...
c
        call ADIdiff3(maxmx, maxmy, maxmz, mx, my, mz, meqn, mbc, maux,
     &                mwork,q,aux,
