      subroutine cellave(xlow,ylow,zlow,dx,dy,dz,w)
      implicit none

      external fs
      real  xlow,ylow,zlow,dx,dy,dz, w

c     # Three-dimensional cellave.f  written by Donna Calhoun, 2005.
c
...
c
c         real(kind=2) function fdisc(x,y,z)
c
...
c     # calling 'cellave' for each mesh cell should be okay.

      integer icube(0:7)
      real edges(0:7,0:7)

      integer ipt, ipt_cnt, iedge, ipt1, ipt2, i,j
      real xl,xh,yl,yh,zl,zh, tol, s
      real corners(0:7,3)
...
c     # Computing area of polygon at x = 1
      real isect_list(10,3)
      real ypoly(10), zpoly(10), area
      integer iv_list(5)
...
c     # For triangulation of surface
      real p0(3), p1(3), p2(3)
      real dw, tri_normal(3), v1(3), v2(3)
      integer sgn, idir
...
c        # be no intersection at that edge.
         ...
         w = w + sgn*sign(1,tri_normal(idir+1))
     &         *tri_normal(1)*dw
      enddo
